...
The floor claim itself is stated canonically in `03-the-floor.md` and is not restated here:

<!-- ddd:ref id=term:floor -->

That result — zero where the predicate closes (path-degeneracy making it *robustly* zero), non-zero
where it does not, and whether it closes itself undecidable (Rice) — is `03`'s. What §2.1–§2.2 add
is why it *moved there* and how path-degeneracy interacts with generation cost.
...
## 4. Composite actors and the compound

An actor may be composed of actors — the **seam-demand identity** applied one level down, to actors
rather than tasks. Composite actors, seam occupancy across the four stores, the actor-vs-mechanism
trade at the seam, and the compound loop that harvests seam-judgment into seam-encoding are stated
in full in `06-composition.md`, which is their canonical home. They are cited here only as the
machinery the re-indexing below draws on.
